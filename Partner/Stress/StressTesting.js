  ...
  if (random < T.signIn) {
    group('signIn group', () => {
      const response = http.post(
        ...
        JSON.stringify({ email: 'walace@yopmail.com', password: 'Confidence@9' }),
    ...
    group('verify group', () => {
      const response = http.post(
        ...
        JSON.stringify({ 'email': 'walace@yopmail.com', 'otp': '111111' }),
        { headers, tags: { endpoint: '/v1/partners/auth/sign-in/verify' } }
      );
      check(response, {
        '/v1/partners/profile: status is 401': (r) => r.status === 401,
      });
    });
    ...
    group('verify group', () => {
      const response = http.post(
        ...
        JSON.stringify({ email: 'walace@yopmail.com' }),
